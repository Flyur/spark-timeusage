...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def isPrimaryNeeds(t: String): Boolean = Set("t01", "t03", "t11", "t1801", "t1803") exists (s => t.trim().startsWith(s))

    def isWorkingActivities(t: String): Boolean = Set("t05", "t1805") exists (s => t.trim().startsWith(s))

    def isOtherActivities(t: String): Boolean =
      Set("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18") exists (s => t.trim().startsWith(s))

    @scala.annotation.tailrec
    def matchByType(columnNames: List[String],
                    primary: List[Column],
                    working: List[Column],
                    other: List[Column]): (List[Column], List[Column], List[Column]) = columnNames match {
      case Nil => (primary, working, other)
      case List() => (primary, working, other)
      case head :: tail =>
        if (isPrimaryNeeds(head)) matchByType(tail, new Column(head) :: primary, working, other)
        else if (isWorkingActivities(head)) matchByType(tail, primary, new Column(head) :: working, other)
        else if (isOtherActivities(head)) matchByType(tail, primary, working, new Column(head) :: other)
        else matchByType(tail, primary, working, other)
    }

    matchByType(columnNames, List(), List(), List())
  }
